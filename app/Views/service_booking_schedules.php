...
<?php
$min_date   = date('Y-m-d');
$max_date   = date('Y-m-d', strtotime('+90 day'));
                ...
                <div class="row">
                    <div class="col-12 col-md-6 col-lg-4 col-xl-3 px-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="date_from" name="date_from" value="" min="<?= $min_date ?>" max="<?= $max_date ?>">
                        </div>
                        <div class="mb-3">
                            <label for="date_to" class="form-label"><?= lang('System.form.filter.date_to') ?></label>
                            <input type="date" class="form-control" id="date_to" name="date_to" value="" min="<?= $min_date ?>" max="<?= $max_date ?>">
                        </div>
                        <div class="mb-3">
                    ...
                    success: function (response) {
                        ...
                        if (null !== response.sessions) {
                            ...
                            $.each(response.sessions, function (i, data) {
                                ...
                                template += '<div class="row"><div class="col-6 text-end"><b><?= lang('System.results.price') ?></b></div><div class="col-6" id="result-actual-price"><?= format_price($variant['price_active'] ?? 0, $business['currency_code']) ?></div></div>';
                                ...
                                template += '<div class="row"><div class="col-12"><button class="btn btn-dark btn-add-to-cart w-100 mt-3" data-session-id="' + data.id + '" data-unit-price="<?= number_format($variant['price_active'] ?? 0, 2, '.', '') ?>" data-short-description="' + data.short_description + '" data-date-start="' + data.date_start + '" data-date-end="' + data.date_end + '"><?= lang('System.results.btn-book') ?></button></div></div>';
