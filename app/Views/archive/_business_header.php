<div class="container mb-3" data-aos="fade-up">
    <div class="row">
        <div class="col">
            <div class="header-div mb-3" style="background-image: url(<?= $business['business_header'] ?>);"></div>
            <img class="img-thumbnail float-start page-logo me-3" src="<?= $business['business_logo'] ?>" alt="<?= $business['business_name'] ?>">
                ...
                <?php if (is_array($business['social_media'])) : ?>
                    <?php foreach ($business['social_media'] as $social_key => $social_link) : ?>
                        <?php if (!empty($social_link)) : ?>
                            <a class="btn btn-outline-dark me-2" href="<?= $social_link ?>" target="_blank"><i class="fa-brands fa-<?= $social_key ?>"></i></a>
                        <?php endif; ?>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
