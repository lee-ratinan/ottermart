<div class="container px-0" data-aos="fade-up">
    <div class="row">
        ...
        <div class="col-12 <?php if (!empty($business['business_header'])) : ?>col-lg-6<?php endif; ?> p-4 pt-5">
            <img class="img-thumbnail page-logo me-3 mb-3" src="<?= $business['business_logo'] ?>" alt="<?= $business['business_name'] ?>">
                ...
                <?php if (is_array($business['social_media'])) : ?>
                    <?php foreach ($business['social_media'] as $social_key => $social_link) : ?>
                        <?php if (!empty($social_link)) : ?>
                            <a class="btn btn-outline-dark rounded-pill me-2" href="<?= $social_link ?>" target="_blank" title="<?= strtoupper($social_key) ?>"><i class="bi bi-<?= $social_key ?>"></i></a>
                        <?php endif; ?>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
